...
pub trait DyPluggable {
    /// 动态插件插入
    fn dyn_plug(&self, lib: Library, lua: &Lua, value: LuaValue) -> Result<()>;
}
/// 加载插件
pub type LoadFn = extern "Rust" fn(ctx: Arc<RawContext>, global: Kissa) -> Result<()>;
/// 创建插件
pub type CreateFn = extern "Rust" fn(lua: &Lua, value: LuaValue) -> Result<Arc<dyn KAny>>;
